...
import {Db} from '../db'
import {fileGetContents} from "../utils";
...
class SqlP {
    ...
    private #prefix = process.env.DB_PREFIX ?? '';
    ...
    constructor(db = Db.getInstance()) {
        ...
        const modelFile = join( __dirname, process.env.DB_ENGINE ?? '', '.php');
        this.loadModel(fileGetContents(modelFile), this.model);
    }

    loadModel(text: string, config): string {
    const regex = new RegExp('/%(.+?)_start%(.+?)%\1_end%/ms');

    const text = preg_replace_callback(regex, (matches) => {
